...
def geminicevir(basliklar):
    ...
    client = genai.Client(api_key="AIzaSyBT5GcrggnQHWjqp2rO16rViPvAwfJrWBE")
...
def trendcevir(limit=15):
    ...
    for i in veri:
        if i[4]!="TR":
            ...
            c.execute("SELECT rowid,* FROM haberler WHERE trend_id=? AND basliktr IS NULL OR basliktr='' ",(i[0],))
            veri2=c.fetchall()
...
def habercevir(haber_id):
    ...
    if veri[0]==None:
        ...
        genai.configure(api_key="AIzaSyBT5GcrggnQHWjqp2rO16rViPvAwfJrWBE")
...
def gununozeti(gun=""):
    ...
    genai.configure(api_key="AIzaSyBT5GcrggnQHWjqp2rO16rViPvAwfJrWBE")
    model= genai.GenerativeModel("gemini-2.0-flash")
    response= model.generate_content (f" {str(haberler)} ---> bu günün haberlerini inceler ve detaylı bir türkçe günün özeti çıkar")
